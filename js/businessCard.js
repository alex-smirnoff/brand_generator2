...
workPhoneInput.addEventListener("focusin", () => {
    if (document.querySelector(".pageLanguage") === "UKR") {
...
function loadConnections(assetsList, webPagesOfAssetsList) {
    changeCardContent(dropdownInputOrganization, companyInCard, "Підприємство", "Enterprise")
...
function showInfoMessage (messageDiv, varBackgroundColor, textNeeded, textParagraph, varTextUKR, varTextEng) {
    ...
    if (textNeeded === true) {
        textParagraph.textContent = document.querySelector(".pageLanguage") === "UKR" ? varTextUKR : varTextEng;
    }
}
